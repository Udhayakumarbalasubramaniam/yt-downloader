...
:root {{
    --surface: rgba(10,10,18,0.82);
    --surface2: rgba(20,20,32,0.88);
    --accent-yt: #ff3c3c;
    --accent-ig: #e1306c;
    --accent-ig2: #f77737;
    --accent-mp3: #a78bfa;
...
.plat-btn:hover {{ border-color: rgba(255,255,255,0.25); color: var(--text); }}
.plat-btn.active-yt {{
...
def download_video(url, format_id, out_dir, title, ph, bar_type):
...
def download_instagram(url, out_dir, title, ph):
    """Download Instagram reel/post — best quality video."""
...
if is_instagram:
    st.markdown("""
    <div class="ig-note">
        📌 &nbsp;Supports: <b>Reels · Posts · IGTV · Stories</b><br>
        🔒 &nbsp;Public posts only — private accounts require login cookies.<br>
        🎵 &nbsp;MP3 extraction also works on Instagram videos.
    </div>
    """, unsafe_allow_html=True)
...
if url:
    with st.spinner("Fetching info…"):
        try:
            ...
            if is_instagram:
                st.info("For private Instagram accounts, you need to export browser cookies. See README for details.")
...
if info:
    ...
    with col2:
        ...
        <div class="info-card {card_cls}">
            <h4>Title</h4>
            <p>{info.get('title') or info.get('description', 'Untitled')[:60]}</p>
        ...
        c1, c2   = st.columns(2)
        with c1:
            st.markdown(f"""<div class="info-card {card_cls}"><h4>Duration</h4><p>{dur_str}</p></div>""", unsafe_allow_html=True)
        with c2:
            lbl = "Account" if is_instagram else "Channel"
            st.markdown(f"""<div class="info-card {card_cls}"><h4>{lbl}</h4><p>{uploader}</p></div>""", unsafe_allow_html=True)

    st.markdown('<hr class="divider">', unsafe_allow_html=True)

    # ── Quality selector (YouTube only, video mode) ──
    ...
    if not is_audio and not is_instagram:
        ...
        st.markdown(f"""<div class="info-card ig"><h4>Quality</h4>
            <p>Best Available (auto)</p></div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""<div class="info-card mp3"><h4>Audio Quality</h4>
    ...
    if st.button("⬇  DOWNLOAD NOW"):
        ...
        try:
            if is_audio:
                file_path = download_audio(url, tmp_dir, title, ph, bar_type)
                mime      = "audio/mpeg"
                dl_name   = sanitize_filename(title) + ".mp3"
            elif is_instagram:
                file_path = download_instagram(url, tmp_dir, title, ph)
                mime      = "video/mp4"
            ...
            ext_label = "MP3" if is_audio else "MP4"
            st.download_button(
                label=f"💾  SAVE  {ext_label}",
                data=data,
                file_name=dl_name,
                mime=mime,
            )
    ...
    if not url:
        ...
        <div style="text-align:center; padding:3rem 0;">
            ...
            <p style="font-family:'Space Mono',monospace; font-size:0.72rem;
                ...
                Paste a {'Instagram' if is_instagram else 'YouTube'} URL above to begin
            </p>
        </div>
        """, unsafe_allow_html=True)
